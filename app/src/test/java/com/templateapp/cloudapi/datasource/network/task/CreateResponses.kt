...
object CreateResponses {
    ...
    val createSuccess = "{ \"response\": \"${SuccessHandling.SUCCESS_TASK_CREATED}\", \"id\": ${blogId}, \"title\": \"${title}\", \"body\": \"${body}\", \"slug\": \"${slug}\", \"date_updated\": \"${dateUpdated}\", \"image\": \"${image}\", \"username\": \"${username}\" }"

    val createFail_titleMustBe5Chars = "{ \"response\": \"Error\", \"error_message\": \"${ErrorHandling.ERROR_TASK_TITLE_LENGTH}\" }"

    val createFail_bodyMustBe50Chars = "{ \"response\": \"Error\", \"error_message\": \"${ErrorHandling.ERROR_TASK_BODY_LENGTH}\" }"

    val createFail_imageTooLarge = "{ \"response\": \"Error\", \"error_message\": \"${ErrorHandling.ERROR_TASK_IMAGE_SIZE}\" }"

    val createFail_imageAspectRatio = "{ \"response\": \"Error\", \"error_message\": \"${ErrorHandling.ERROR_TASK_IMAGE_ASPECT_RATIO}\" }"
